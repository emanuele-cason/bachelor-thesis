<div align="center">
  <img src="Presentazione/img/logo-unipd.png" alt="University of Padua" height="80">
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <img src="Presentazione/img/logo-dii.png" alt="Department of Industrial Engineering" height="80">

  # Raccolta e analisi dei dati di volo per un velivolo a controllo remoto

  **Logging and analysis of flight data for a remotely controlled aircraft**
  
  Bachelor's Degree in Aerospace Engineering — Università degli Studi di Padova
  
  [![Language](https://img.shields.io/badge/Language-Italian-blue.svg)](#)
  [![Course](https://img.shields.io/badge/Course-Aerodinamica_1-orange.svg)](#)
  [![Official Publication](https://img.shields.io/badge/Thesis-Padua_Research_Archive-lightgrey.svg)](https://thesis.unipd.it/handle/20.500.12608/48867)
</div>

## Available Materials

- [Thesis Document (PDF)](https://raw.githubusercontent.com/emanuele-cason/bachelor-thesis/master/Tesi.pdf)
- [Presentation (PDF)](https://raw.githubusercontent.com/emanuele-cason/bachelor-thesis/master/Presentazione/Presentazione%20relazione.pdf)
- [Official Publication on Padua Research Archive](https://thesis.unipd.it/handle/20.500.12608/48867)

---

## Abstract

During the development and testing of the remotely piloted aircraft for the "Air Cargo Challenge" international competition, there was the need to obtain flight data from the model aircraft to allow an evaluation of its performance. The experimental approach to obtain such information consisted of two main aspects: the integration of a flight data recording system and transmission of this data to the ground (telemetry); and the analysis of the collected data, through a custom-created software. This analysis allowed to make a comparison between project-predicted performance and empirical data, to identify the optimal flight and maneuvering conditions and to estimate competition scores.

---

## Estratto (Italiano)

Con lo sviluppo e il collaudo del velivolo a pilotaggio remoto, nell'ambito della competizione internazionale "Air Cargo Challenge", si è reso necessario ottenere dei dati di volo dall'aeromodello per consentire una valutazione delle sue prestazioni. L'approccio sperimentale per ottenere tali informazioni si è articolato in due aspetti principali: l'integrazione di un sistema di registrazione dei dati di volo e trasmissione di questi ultimi a terra (telemetria); e l'analisi dei dati raccolti, attraverso un software creato appositamente. Tale analisi ha permesso di operare un confronto tra previsioni teoriche di progetto e dati empirici, di individuare le condizioni ottimali di volo e di manovra e prevedere in linea di massima i punteggi di gara.
